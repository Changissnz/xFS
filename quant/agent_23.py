...
"""
The modus operandi container for class<AgentType2F3M>. 
"""
class AgentType2F3MMOContainer: 
    ...
    """
    generates a dataset D w/ `samples_per_label` samples for every label except for `ref_label`, 
    which has 1 + `samples_per_label` samples (including the `ref_vec`). 

    Every sample is produced through adding a vector output from `prg` to `ref_vec`. 
    """
    @staticmethod
    ...
    #----------------------------------------------- methods to generate instances of this class 
...
Agent is to be used alongside two other agents of the same class<AgentType2F3M>. All 
three agents operate by the same modus operandi, 'compatible characterization' (CC) XOR  
'third-party contra' (3PC). Additionally, all three agents decide and act for the same 
category-to-labels map M: 
    M : category -> list::labels. 


Agent is used in weighted processes that determine whether it moves by its own "characterization" 
or not. 

At every timestamp, agent moves by a three-step procedure: 
I) preprocessing: characterize self and other two agents. 
II) processing: weigh out the characterization of self and from others. 
III) execute: act by decision derived from step II. 

"Characterization" is defined for this agent as its using its PRNG to select exactly one label 
for each category to characterize itself, and likewise for the other two agents. See 
method<AgentType2F3MMOContainer.characterize_agent> and method<AgentType2F3MMOContainer.characterize_self>. 
...
For step 2:
    - CC
- Agent A "justifies" its categorical characterizations of each of the two other agents. 
- "Justification" is a classification process for every category T, s.t. given other agent B, 
   agent A uses the reference vector V_ref that is <B.attribute_vec>. Agent A then calculates an additional 
   |<A.mo_container.c2l_map[T]>| * <A.mo_container.cc_justification_l2s_size> samples for dataset D, each 
   sample the sum V_ref + V_q; V_q a vector of length |V_ref| generated by A's PRNG. A then classifies every 
   sample of D. The justification strength is the unweighted classification accuracy of D w.r.t. those sample's 
   labels of T.  

   - 3PC 
- For triangulated permission negotiation. 
- For every category T, Agent A determines exactly one of the other two agents B and C to serve as the first 
  relay for its selection l of T. The first relay informs the second relay, the other of B and C, of A's selection. 
  
    *note*
  The first relay's decision, a float in [0.,1.], to accept A's selection l of T carries twice the weight of the second 
  relay's decision.  
...
- For every category T, 
    - Agent A normalizes the probabilities associated with three labels of T, one from A and two from the other two, B 
      and C. 
    - The probability associated with A's own label is the average non-compatibility it has with the other two agents: 
        ``` 
        ((1 - self.mo_container.comp_map[B]) + (1 - self.mo_container.comp_map[C])) / 2.0 
        ```
      The probabilities associated with B's and C's labels for A are those labels' justification strengths. 
...
- For every category T, 
    ...
    - The boolean of approval from the combined influence of the first relay R_0 and the second relay R_1 
      is Q = (d1 <= x0 + x1): 
      - x0: 
        2 * B_R0 * self.mo_component.comp_map[R0] * self.mo_component.aa_comp_map[R0][T][l]
      - x1:
        B_R1 * self.mo_component.comp_map[R1] * self.mo_component.aa_comp_map[R1][T][l]
    - Agent A cannot act by l for T if Q is False. 
"""
class AgentType2F3M: 
    ...
    """
    sets uniform compatibility for exactly one of: 
    - self-to-other-agent general relation,  
    - other agent's (category,label). 
    """
    ...
    #--------------------------- processing phase: weigh characterization of self and from others 
    ...
    def process_one__3PC_(self,a1,a2,category):
        ...
        stat = first_relay.mo_container.recv_action_leak(self,second_relay,category)
